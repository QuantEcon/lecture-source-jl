...
.. code-block:: julia
    ...
    function mc_sample_path(P; init=1, sample_size=1000)
        ...
        n = size(P)[1]
        P_dist = [DiscreteRV(vec(P[i,:])) for i ∈ OneTo(n)]
...
.. code-block:: julia
    ...
    function tmp()
        ...
        t = 20
        x_vals = Vector{Float64}(undef, t+1)
        y_vals = Vector{Float64}(undef, t+1)
        z_vals = Vector{Float64}(undef, t+1)
        colors = []
...
.. code-block:: julia
    ...
    function tmp()
        ...
        for x0 ∈ OneTo(2)
            ...
            X_bar = cumsum(X.==1) ./ (collect(OneTo(1:N)))
...
.. code-block:: julia
    ...
    function tmp()
        ...
        ranked_pages = Dict(alphabet[i] => r[i] for i ∈ OneTo(n))
